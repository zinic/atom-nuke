...
import net.jps.nuke.listener.eps.eventlet.AtomEventletPartial;
...
public class EPSMain {
   ...
   public static void main(String[] args) throws Exception {
      ...
      final Task task1 = nukeKernel.tasker().follow(new EventGenerator("Task 1", true), new TimeValue(500, TimeUnit.MILLISECONDS));
      task1.addListener(relay1);

      final Task task2 = nukeKernel.tasker().follow(new EventGenerator("Task 2", true), new TimeValue(1, TimeUnit.SECONDS));
      task2.addListener(relay1);
      task2.addListener(relay2);

      final Task task3 = nukeKernel.tasker().follow(new EventGenerator("Task 3", true), new TimeValue(2, TimeUnit.SECONDS));
